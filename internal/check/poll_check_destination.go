...
// PollCheckDestination checks the given destination
//
// POST /api/<username>/<server>/cd/poll
...
func PollCheckDestination(phone, service string) {
	// Get epoch time
	...
	resp, err := http.PostForm(constants.POLL_CHECK_DESTINATION_URL, data)
	if err != nil {
		log.Fatal(err)
	}
